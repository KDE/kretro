...
ToolBar {
    ...
    RowLayout {
        ...
        ToolButton {
            id: menuButton
            visible: Kirigami.ApplicationWindow.menu.visible
            icon.name: "open-menu-symbolic"
            text: i18n('Open Sidebar')
            display: AbstractButton.IconOnly
            onClicked: {
                applicationWindow().mainMenu.parent = menuButton
                applicationWindow().mainMenu.y = menuButton.height
                applicationWindow().mainMenu.open()
            }
        }
